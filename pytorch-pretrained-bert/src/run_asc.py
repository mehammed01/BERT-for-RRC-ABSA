...
def train(args):
    ...
    tokenizer = ABSATokenizer.from_pretrained("bert-base-arabertv02")
    print(5)
    ...
    model = BertForSequenceClassification.from_pretrained("bert-base-arabertv02", num_labels = len(label_list) )
...
def test(args):  # Load a trained model that you have fine-tuned (we assume evaluate on cpu)    
    processor = data_utils.AscProcessor()
    label_list = processor.get_labels()
    tokenizer = BertTokenizer.from_pretrained("bert-base-arabertv02")
